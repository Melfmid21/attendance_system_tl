...
<body class="bg-content">
    ...
    <div class="p-4 sm:ml-64">
        <main class="p-2 dark:border-gray-700 mt-8">
            <div id="content-area">
                <h2 class="text-menu text-2xl font-bold mb-4 pt-4">College Subject</h2>
                <div class="bg-white px-6  rounded-sm shadow-sm">
                    ...
                    <div class="flex justify-between items-center pt-4 pb-2  bg-white">
                        ...
                        <div>
                            ...
                            <input type="text" id="studentSearch" placeholder="Search subject & press enter..."
                                class="p-2 border border-gray-300 rounded-sm focus:ring-2 focus:ring-gray-200 focus:outline-none w-64">
                        </div>

                        <button id="addSubject" data-modal-target="add-subject-modal"
        ...
        try {
            ...
            // Query to fetch subject data with pagination
            $stmt = $pdo->prepare("
            ...
            // Query to count total number of subjects for pagination
            $countStmt = $pdo->query("SELECT COUNT(*) FROM subjects WHERE grade_level = 'college' AND isDeleted = 0");
            $totalSubjects = $countStmt->fetchColumn();
            ...
            if ($subjects) {
                foreach ($subjects as $subject) {
                    ...
                    echo '<td class="px-6 py-4 text-gray-800 whitespace-nowrap dark:text-white">' . htmlspecialchars($subject['id']) . '</td>';
                    echo '<td class="px-6 py-4 text-gray-800">' . htmlspecialchars($subject['subject_name']) . '</td>';
                    echo '<td class="px-6 py-4 text-gray-800 flex items-center gap-2">';
                    
                    // Edit button
                    echo '<button class="showEditSubjectModal inline-flex items-center p-2 rounded-full transition-all duration-200 ease-in-out hover:bg-blue-100" 
                    data-modal-target="edit-subject-modal" 
                    data-modal-toggle="edit-subject-modal" 
                    data-subject-id="' . htmlspecialchars($subject['id'], ENT_QUOTES, 'UTF-8') . '" 
                    ...
                    // Delete button
                    echo '<button class="deleteSubject inline-flex items-center p-2 rounded-full transition-all duration-200 ease-in-out hover:bg-red-100" 
                            data-subject-id="' . htmlspecialchars($subject['id'], ENT_QUOTES, 'UTF-8') . '">
                ...
                echo '<tr><td colspan="3" class="px-6 py-4 text-center">No subjects found</td></tr>';
            }
            ...
            echo '<nav class="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4 px-4 pb-2" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400 mb-4 md:mb-0 block w-full md:inline md:w-auto">
                        Showing <span class="font-semibold text-gray-900 dark:text-white">' . (($page - 1) * $limit + 1) . '-' . min($page * $limit, $totalSubjects) . '</span>
                        of <span class="font-semibold text-gray-900 dark:text-white">' . $totalSubjects . '</span>
                    </span>
        ...
        ?>
                ...
                <?php  include '../../include/admin/add_modal_subject_college.php';?>
                <?php  include '../../include/admin/edit_modal_subject_college.php';?>
        ...
        if (form) {
            form.addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    const response = await fetch('../../api/admin/api_add_subject.php', {
                        method: 'POST',
                        body: formData
                    });
                    ...
                    Swal.fire({
                        title: 'Error!',
                        text: 'An error occurred while adding the subject: ' + error
                            .message,
                        icon: 'error',
                        confirmButtonText: 'OK'
                    });
    ...
    //SCRIPT TO EDIT SUBJECT
    document.addEventListener('DOMContentLoaded', function() {
        // Get all edit buttons
        const editButtons = document.querySelectorAll('.showEditSubjectModal');
        const editForm = document.getElementById('edit-subject-form');

        // Variable to store original subject name for comparison
        ...
        editButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Store original subject name for later comparison
        ...
        if (editForm) {
            editForm.addEventListener('submit', async function(e) {
                ...
                const currentSubjectName = document.getElementById('edit_subject_name').value
                    .trim();

                // Check if there are any changes
                if (currentSubjectName === originalSubjectName) {
                    Swal.fire({
                        title: 'No Changes',
                        text: 'No changes were made to the subject name.',
                        icon: 'info',
                        confirmButtonText: 'OK'
                    });
                ...
                try {
                    ...
                    const response = await fetch('../../api/admin/api_update_subject.php', {
                        method: 'POST',
                        body: formData
                    });
                    ...
                    Swal.fire({
                        title: 'Error!',
                        text: 'An error occurred while updating the subject: ' + error
                            .message,
                        icon: 'error',
                        confirmButtonText: 'OK'
                    });
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const deleteButtons = document.querySelectorAll('.deleteSubject');
        ...
        async function deleteSubject(subjectId) {
            try {
                ...
                const response = await fetch('../../api/admin/api_delete_subject.php', {
                    method: 'POST',
                    body: formData
                });
